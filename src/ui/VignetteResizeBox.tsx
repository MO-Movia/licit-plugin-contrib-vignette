...
import './czi-vignette-resize-box.css';
...
class VignetteResizeBoxControl extends React.PureComponent {
  props: {
    boxID: string,
    config;
    direction: string,
  ...
  render(): React.ReactElement {
    ...
    const className = cx({
      'molv-czi-vignette-resize-box-control': true,
      [direction]: true,
    });
  ...
  _syncSize = (): void => {
    ...
    let ww = clamp(MIN_SIZE, Number(width) + Math.round(dx), MAX_SIZE);
    let hh = clamp(MIN_SIZE, Number(height) + Math.round(dy), MAX_SIZE);
  ...
  _end(): void {
    ...
    el.className = 'molv-czi-vignette-resize-box';
    this._el = null;
...
class VignetteResizeBox extends React.PureComponent {
  props: Props;
  ...
  render(): React.ReactElement {
    ...
    return (
      <span className="molv-czi-vignette-resize-box" id={boxID} style={style}>
        {controls}
      </span>
    );
  }
...
export default VignetteResizeBox;
